...
function registration($name, $email, $password, $conn) {
        ...
        $data['success'] = true;
            ...
            if(empty($email)){
                $data['success']  = false;
                $data['error_msg'] = "ENTER - YOUR e-Mail ADDRESS";
                echo json_encode($data);
                return true;
            } else if (empty($name)) {
                $data['success']  = false;
                $data['error_msg'] = "ENTER - YOUR NAME";
                echo json_encode($data);
                return true;
            } else if ( !filter_var($email,FILTER_VALIDATE_EMAIL) ) {
                $data['success']  = false;
                $data['error_msg'] = "ENTER - VALID e-Mail ADDRESS";
                echo json_encode($data);
                return true;
            }
            ...
            if(empty($pass)){
                $data['success']  = false;
                $data['error_msg'] = "ENTER - YOUR PASSWORD";
                echo json_encode($data);
                return true;
            }
            ...
            if ($data['success']) {
                ...
                if($count == 1){
                    $data['success']  = false;
                    $data['error_msg'] = "Sorry! - e-Mail ALREADY EXISTS";
                    echo json_encode($data);
                    return true;
                }
                ...
                if($result) {
                    ...
                    $data['success']  = false;
                    $data['error_msg'] = "INCORRECT INFO - TRY AGAIN!";
                    echo json_encode($data);
                    return true;
                }
